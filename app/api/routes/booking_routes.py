...
#         booking_crud.update_booking_status(db, booking, BookingStatus.cancelled, None)
...
def get_current_identity(credentials: HTTPAuthorizationCredentials = Depends(security)) -> Optional[Union[dict, None]]:
    """
    ...
    try:
        ...
        if not token_type or not sub_id:
            return None
        
        # Fetch full details from DB
        db = next(get_db())  # Temporary DB session for validation
        if token_type == 'user':
            user = user_crud.get_user_by_id(db, sub_id)  # Assuming user_crud exists
            if user:
                return {
                    'type': 'user',
                    'id': user.id,
        ...
        elif token_type == 'vendor':
            vendor = service_provider_crud.get_vendor_by_id(db, sub_id)  # Assuming vendor_crud exists
            if vendor:
                return {
                    'type': 'vendor',
                    'id': vendor.id,
