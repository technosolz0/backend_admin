...
from app.crud.service_provider_crud import (
    ...
    vendor_login, get_all_vendors, delete_vendor
)
from app.database import SessionLocal
from app.schemas.category_schema import CategoryOut
from app.schemas.sub_category_schema import SubCategoryOut
        ...
        transformed_vendors = [
            {
                ...
                "serviceId": str(vendor.subcategory_charges[0].subcategory_id) if vendor.subcategory_charges else "",  # Assuming serviceId maps to subcategory_id
                "serviceName": db.query(SubCategory).filter(SubCategory.id == vendor.subcategory_charges[0].subcategory_id).first().name if vendor.subcategory_charges else "N/A",  # Assuming serviceName maps to subcategory name
...
def get_me(current_vendor: ServiceProvider = Depends(get_current_vendor), db: Session = Depends(get_db)):
    ...
    try:
        return {
            "id": str(current_vendor.id),
            "name": current_vendor.full_name,
            "categoryId": str(current_vendor.category_id) if current_vendor.category_id else "",
            "categoryName": db.query(Category).filter(Category.id == current_vendor.category_id).first().name if current_vendor.category_id else "N/A",
            "subcategoryId": str(current_vendor.subcategory_charges[0].subcategory_id) if current_vendor.subcategory_charges else "",
            "subcategoryName": db.query(SubCategory).filter(SubCategory.id == current_vendor.subcategory_charges[0].subcategory_id).first().name if current_vendor.subcategory_charges else "N/A",
            "serviceId": str(current_vendor.subcategory_charges[0].subcategory_id) if current_vendor.subcategory_charges else "",
            "serviceName": db.query(SubCategory).filter(SubCategory.id == current_vendor.subcategory_charges[0].subcategory_id).first().name if current_vendor.subcategory_charges else "N/A",
            "contactInfo": current_vendor.email or current_vendor.phone or "N/A",
            "status": current_vendor.admin_status.capitalize()
        }
    except Exception as e:
