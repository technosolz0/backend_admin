...
def generate_referral_code(db: Session, full_name: str, length=4) -> str:
    ...
    while True:
        ...
        exists = db.query(Vendor).filter(Vendor.referral_code == code).first()
        if not exists:
            return code
...
def build_vendor_response(db: Session, vendor: Vendor) -> VendorResponse:
    ...
    if vendor.referred_by_id:
        referrer = db.query(Vendor).filter(Vendor.id == vendor.referred_by_id).first()
        if referrer:
    ...
    # Referrals made
    referrals_made = []
    referrals = db.query(Vendor).filter(Vendor.referred_by_id == vendor.id).all()
    for r in referrals:
...
def get_all_vendors(db: Session, page: int = 1, limit: int = 10, search: Optional[str] = None, status: Optional[str] = None):
    ...
    try:
        ...
        query = (
            db.query(Vendor)
            .options(
                joinedload(Vendor.category),
                joinedload(Vendor.subcategory_charges).joinedload(VendorSubcategoryCharge.subcategory)
            )
        )
        ...
        if status:
            query = query.filter(Vendor.admin_status == status)

        total = query.count()
        vendors = query.order_by(Vendor.created_at.desc()).offset(offset).limit(limit).all()
...
def verify_vendor_otp(db: Session, email: Optional[str] = None, otp: Optional[str] = None, phone: Optional[str] = None) -> Dict[str, Any]:
    """Verify OTP for vendor verification."""
    try:
        vendor = None
        if email:
            vendor = get_vendor_by_email(db, email)
        if not vendor and phone:
            vendor = db.query(Vendor).filter(Vendor.phone == phone).first()
        
        if not vendor:
            logger.warning(f"OTP verification attempt for non-existent account: email={email}, phone={phone}")
            return {
                "success": False,
